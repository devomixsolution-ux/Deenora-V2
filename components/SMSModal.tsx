...
const SMSModal: React.FC<SMSModalProps> = ({ students, madrasah, lang, onClose, onSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#080A12]/40 backdrop-blur-2xl z-[500] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-sm rounded-[3rem] shadow-2xl p-8 border border-[#8D30F4]/5 animate-in zoom-in-95 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 bg-[#8D30F4]/5 rounded-xl flex items-center justify-center text-[#8D30F4]"><MessageSquare size={24} /></div>
          <div><h2 className="text-xl font-black text-slate-800 font-noto leading-tight">এসএমএস পাঠান</h2><p className="text-[9px] font-black text-[#8D30F4]/60 uppercase tracking-widest mt-1">{students.length} জন ছাত্র নির্বাচিত</p></div>
        </div>
        <div className="space-y-5">
          <div className="relative"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1 mb-2 block">টেমপ্লেট বেছে নিন</label><button onClick={() => setShowTemplateDropdown(!showTemplateDropdown)} className="w-full flex items-center justify-between px-5 py-4 bg-slate-50 border border-slate-100 rounded-xl text-slate-600 text-sm font-bold shadow-sm"><div className="flex items-center gap-2"><BookOpen size={16} className="text-[#8D30F4]" /><span className="truncate">Saved Templates</span></div><ChevronDown size={18} className={`text-slate-300 transition-all ${showTemplateDropdown ? 'rotate-180' : ''}`} /></button>
          ...
          <div className="space-y-2"><div className="flex justify-between items-center px-1"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">বার্তা লিখুন</label><span className="text-[9px] font-black text-[#8D30F4] uppercase tracking-widest">{message.length}/160</span></div><textarea className="w-full h-28 px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none text-slate-700 font-medium text-sm focus:border-[#8D30F4]/30 resize-none shadow-inner leading-relaxed" placeholder="মেসেজ লিখুন..." value={message} onChange={(e) => setMessage(e.target.value)} maxLength={160} /></div>
